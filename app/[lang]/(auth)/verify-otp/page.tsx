...
export default function OTPVerificationPage() {
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      dispatch(authAction.getUser(res?.response?.user));
  ...
  }, [allFilled, code, secret, target, type, userLogin, dispatch, language, L]);
  ...
  const onResend = async () => {
    ...
    try {
      const res = await sendUserOtp(payload as any).unwrap();

      setTimeout(() => {
        toast.success(L.codeSent);
        router.push(
          `/${language}/verify-otp?type=${type}&target=${target}&secret=${res?.response?.secret_code}`
        );
      }, 300);
    } catch (err: any) {
